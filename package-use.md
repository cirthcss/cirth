...
It is called by `scripts/build.js` through the Sass Embedded Node API. The build script compiles every non-partial `.scss` entry under `src/` into the matching `.css` file under `dist/`.
...
3. Compile base CSS with `sass-embedded`.
4. Compile theme CSS with `scripts/build-themes.js`.
5. Transform compiled CSS with Lightning CSS.
6. Generate minified CSS with Lightning CSS.
...
The `dist/` directory contains generated output. When changing SCSS sources or build tooling, run:

```sh
npm run build
```

Then review the generated CSS diff before committing.

Large CSS diffs can be normal when changing the processor, minifier, browser targets, or Sass output style. They should still be reviewed for unexpected file additions, missing `.min.css` files, or accidental changes outside the generated CSS surface.
