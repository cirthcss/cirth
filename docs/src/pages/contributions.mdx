...
* `docs/` contains this site (Astro), styled by Cirth's own build. Kept in
  this repository rather than a dedicated one **on purpose, for now**: the
  library is still under heavy, fast-moving revision, and co-locating docs
  with source means a change gets its matching docs update in the same
  commit — checked by the same CI — instead of the two drifting apart
  across separate repos. Once the pace of change settles, the plan is to
  split docs into their own repository and do a full manual review pass
  over everything written so far. Noted here so the current layout isn't
  mistaken for the permanent shape of the project.
