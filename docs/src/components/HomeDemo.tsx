...
// backward, hold, repeat. The full final state is server-rendered (this
// island hydrates on top of it), so no-JS visitors, search engines, and
// screen readers always get the complete content; the animation is a
// purely visual layer applied after mount, and skipped entirely under
...
export default function HomeDemo() {
	const [typed, setTyped] = useState(fullSource);
	const [preview, setPreview] = useState(finalSnapshot);
	...
	useEffect(() => {
		...
		// One full cycle: type each segment (rendering its preview as it
		// lands), hold the finished page, delete backward (the preview
		// un-building the same way), hold the blank pane, then schedule the
		// next cycle.
		const cycle = () => {
			clearTimers();
			setTyped("");
			...
			for (let length = fullSource.length - 1; length >= 0; length--) {
				...
				timers.current.push(
					setTimeout(() => {
						setTyped(fullSource.slice(0, sliceTo));
						const partial = segmentEnds.findIndex((end) => sliceTo < end);
						setPreview(partial <= 0 ? "" : segments[partial - 1].snapshot);
					}, at),
				);
			}

			at += HOLD_EMPTY_MS;
			timers.current.push(setTimeout(cycle, at));
		};
		...
		if (!reduceMotion) {
			setAnimated(true);
			timers.current.push(setTimeout(cycle, 600));
		}
		...
		<div class="home-demo" aria-label="Live example: semantic HTML rendered by Cirth">
			...
			<div class="home-demo-code" data-theme="dark">
				...
				<pre aria-hidden="true">
					<code>
						{typed}
						{animated && <span class="home-demo-cursor" />}
					</code>
				</pre>
			...
			<div class="home-demo-preview" dangerouslySetInnerHTML={{ __html: preview }} />
		</div>
	);
}
