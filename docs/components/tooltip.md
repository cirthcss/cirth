...
* The tooltip text is the `data-tooltip` attribute value itself, rendered
  via CSS `content: attr(data-tooltip)`, with no extra element needed.
...
* On devices with hover and a fine pointer, the tooltip slides in with a short
...
* **It can't be dismissed with <kbd>Esc</kbd>** without moving focus or the
  ...
  page must meet 1.4.13 strictly for elements with tooltips, use a small
  JS tooltip (or the Popover API) instead of this pattern. Cirth's
  styles won't get in the way.
* Keep the text short: the tooltip is a single line without wrapping and
