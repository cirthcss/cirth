...
```json
{
  "exports": {
    ...
    "./presets/cobalt": "./presets/cobalt.css",
    "./presets/coral": "./presets/coral.css",
    "./presets/*": "./presets/*",
    "./dist/*": "./dist/*",
...
API to compile each `src/presets/*.scss` entry point into `presets/*.css`.
...
6. Compile preset CSS with `scripts/build-presets.js` (`src/presets/*.scss` →
   `presets/*.css`).
7. Transform compiled CSS (`dist/` and `presets/`) with Lightning CSS.
8. Generate minified CSS for `dist/` with Lightning CSS. Presets are small
   token overrides and are not published as a separate minified file.
...
The `dist/` and `presets/` directories contain generated output. Both are
ignored by Git and regenerated by local builds, CI, release workflows, and
npm publishing.

The build cleans `dist/` and `presets/` before regenerating CSS, so removed
or renamed entry points do not leave stale files behind. When changing SCSS
sources or build tooling, run:
...
and `presets/` files should not be committed.
