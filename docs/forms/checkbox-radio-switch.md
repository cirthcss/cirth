...
a checkbox turns it into a toggle switch — no extra markup.
...
- Checked state uses `--cirth-primary-background`/`-border` and an inline
  check-mark icon (`--cirth-icon-checkbox`); an indeterminate checkbox shows
