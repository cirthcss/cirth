...
### 4. One flaky behavior test

`framework-specimen.spec.js` "playroom light keeps native highlights, range, and
semantic actions distinct" fails intermittently in Firefox under the full
parallel run and is deterministic in isolation. It reads computed styles from a
freshly navigated specimen, so the likely cause is a race against style
resolution under load rather than anything about the assertion. Diagnose it;
do not paper over it with a local retry, because the flake reporter exists to
make exactly this visible.
