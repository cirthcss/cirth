...
const readSchemes = (css) => {
	...
	root.each((node) => {
		if (node.type === "rule") {
			...
			if (selector === BASE_ROOT) base.push(...customProperties(node));
			if (selector === LIGHT_ROOT) overrides.light = customProperties(node);
			if (selector === DARK_ROOT) overrides.dark = customProperties(node);
		} else if (
			node.type === "atrule" &&
			node.name === "media" &&
			...
			node.each((child) => {
				if (child.type === "rule" && normalize(child.selector) === AUTO_DARK_ROOT) {
					autoDark = customProperties(child);
				}
			});
		}
	});
	...
	return { light: resolve(0, overrides.light), dark: resolve(1, overrides.dark) };
};
...
const parseHex = (hex) => {
	...
	const channels = (full.match(/../g) ?? []).map((pair) => Number.parseInt(pair, 16) / 255);
...
const evaluateCalc = (expression, channels) => {
	const tokens = expression.match(/-?(?:\d+\.?\d*|\.\d+)|[a-z]+|[-+*/()]/g) ?? [];
	let position = 0;
	...
	const value = sum();
	if (position !== tokens.length) throw new Unsupported(`calc(): ${expression}`);
	return value;
};
...
const channel = (token, percentScale, channels, env) => {
	...
	if (call?.name === "var") return env.number(variableName(call.args));
	return parseNumber(token, percentScale);
};

/** @param {string} args */
const variableName = (args) => {
	const match = args.trim().match(/^--cirth-([a-z0-9-]+)$/);
...
const evaluateOklch = (args, env) => {
	...
	if (parts[0] === "from") {
		...
		channels = { l: origin.l, c: origin.c, h: origin.h ?? 0, alpha: origin.alpha };
		parts = parts.slice(2);
	}
	...
	return { l: l ?? 0, c: c ?? 0, h: h === null ? null : ((h % 360) + 360) % 360, alpha: alpha ?? 1 };
};
...
const evaluateColorMix = (args, env) => {
	...
	if (!space || second === undefined) throw new Unsupported(`color-mix(${args})`);
	...
	const operand = (part) => {
		...
		return {
			color: evaluateColor((hasPercent ? pieces.slice(0, -1) : pieces).join(" "), env),
	...
	if (space === "oklab") {
		...
		return oklabToOklch({ l: mix("l"), a: mix("a"), b: mix("b") }, alpha * alphaScale);
	}
	...
	const mix = (key) =>
		alpha === 0
			? 0
			: (a.color[key] * a.color.alpha * p1 + b.color[key] * b.color.alpha * p2) / alpha;
...
const evaluateColor = (value, env) => {
	...
	if (call?.name === "var") return env.color(variableName(call.args));
...
const convertScheme = (declared) => {
	...
	const convert = (name) => {
		...
		if (css === undefined) throw new Unsupported(`--cirth-${name} is not declared on the root`);
		if (visiting.has(name)) throw new Unsupported(`--cirth-${name} refers to itself`);
	...
	const tokenOf = (name) => {
		...
		if (!("$type" in result)) {
			throw new Unsupported(`refers to --cirth-${name}, which has no DTCG type`);
		}
		return result;
	};
	...
	const env = {
		color: (name) => {
			const css = declared.get(name);
			if (css === undefined) throw new Unsupported(`--cirth-${name} is not declared`);
			tokenOf(name); // the referenced token must itself be a colour
			return evaluateColor(css, env);
		},
		number: (name) => {
			const token = tokenOf(name);
			if (token.$type !== "number") throw new Unsupported(`--cirth-${name} is not a number`);
			const css = declared.get(name) ?? "";
			return NUMBER.test(css) ? Number.parseFloat(css) : env.number(variableName(functionCall(css)?.args ?? ""));
		},
	};
	...
	const convertValue = (name, css) => {
		...
		if (call?.name === "var") {
			const target = variableName(call.args);
		...
		if (name.startsWith("font-family")) {
			const families = splitTopLevel(css, ",").flatMap((part) => {
				...
				if (reference?.name === "var") {
					const value = tokenOf(variableName(reference.args)).$value;
		...
		if (duration) {
			return token("duration", { value: Number.parseFloat(duration[1]), unit: duration[2] });
		}
		...
		try {
			const literal = /^(#[0-9a-f]+|black|white|transparent|oklch\((?!from)[^()]*\))$/i.test(css);
		...
		const referenceTo = (part, type) => {
			...
			const target = variableName(reference.args);
		...
		const shadows = splitTopLevel(css, ",").map((layer) => {
			...
			if (lengths.length < 2 || lengths.length > 4 || lengths.some((part) => !part)) {
				throw new Unsupported("no DTCG type holds this CSS value");
			}
			const color = parts[parts.length - 1];
			return {
				color: referenceTo(color, "color") ?? dtcgColor(evaluateColor(color, env)),
				offsetX: lengths[0],
				offsetY: lengths[1],
 ...
 */
const buildTokenDocuments = (css, version) => {
	const schemes = readSchemes(css);

	return Object.fromEntries(
		/** @type {Scheme[]} */ (["light", "dark"]).map((scheme) => {
			const { tokens, unrepresented } = convertScheme(schemes[scheme]);
			...
			return [
				scheme,
				{
					$description:
						...
						"dist/cirth.css; token names drop the --cirth- prefix.",
					$extensions: {
						[EXTENSION]: {
							version,
							scheme,
							...
							unrepresented: Object.fromEntries(
								[...unrepresented.keys()].sort().map((name) => [
									name,
