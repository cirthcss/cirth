...
// transform and minify steps build.js runs, and requires each prefixed
...
check("an invalid prefix is refused", () => {
	for (const bad of ["acme-", "--acme", "--1acme-", "--ac me-", "--", "-acme-"]) {
...
try {
	...
	check(`every build file differs from the default by the prefix alone`, () => {
		for (const file of files) {
			...
			assert.ok(countDefault(expected) > 0, `${file}: no ${DEFAULT_PREFIX} to rename`);
			assert.strictEqual(countDefault(actual), 0, `${file}: ${DEFAULT_PREFIX} left behind`);
			assert.ok(
	...
	check(`${files.length} files, including scoped builds and presets`, () => {
